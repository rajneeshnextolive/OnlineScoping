...
function callOnConnect() {
  ...
  var getCallTimer = $('.overlay-phone-call .timer');
   getCallStatusText.text('Connected');
   getCallTimer.css('visibility', 'visible');
}
...
$('.phone-call-screen').off('click').on('click', function(event) {
  ...
  var getCallingUserImg = $(this).parents('.chat-system').find('.person.active .f-head img').attr('src');
   $(this).parents('.chat-box').find('.overlay-phone-call .user-name').text(getCallingUserName);
   $(this).parents('.chat-box').find('.overlay-phone-call .calling-user-img img').attr('src', getCallingUserImg);
   $(this).parents('.chat-box').find('.overlay-phone-call').addClass('phone-call-show');
...
$('.switch-to-video-call').off('click').on('click', function(event) {
    ...
    $('.overlay-video-call').find('.calling-user-img img').attr('src', getCallerImg);
     $(this).parents('.overlay-phone-call').removeClass('phone-call-show');
    $(this).parents('.overlay-phone-call').find('.call-status').text('Calling...');
    $(this).parents('.overlay-phone-call').find('.timer').removeAttr('style');
    setTimeout(videoCallOnConnect, 2000);
})
$('.switch-to-microphone').off('click').on('click', function(event) {
   $(this).toggleClass('micro-off');
})
$('.cancel-call').on('click', function(event) {
    ...
    if ($(this).parents('.overlay-phone-call').hasClass('phone-call-show')) {
      ...
      $(this).parents('.overlay-video-call').removeClass('video-call-show');
        $(this).parents('.overlay-video-call').find('.call-status').text('Calling...');
       $(this).parents('.overlay-video-call').removeClass('onConnect');
      $(this).parents('.overlay-video-call').find('.calling-user-img').css('display', 'block');
      $(this).parents('.overlay-video-call').find('.timer').removeAttr('style');
    }
})
$('.go-back-chat').on('click', function(event) {

  if ($(this).parents('.overlay-phone-call').hasClass('phone-call-show')) {
    $(this).parents('.chat-box').find('.overlay-phone-call').removeClass('phone-call-show');
  } else if ($(this).parents('.overlay-video-call').hasClass('video-call-show')) {
     $(this).parents('.chat-box').find('.overlay-video-call').removeClass('video-call-show')
  }
...
function videoCallOnConnect() {
  ...
  getVideoCallingDiv.addClass('onConnect');
  ...
  var getCallTimer = $('.overlay-video-call .timer');
      getCallStatusText.text('Connected');
      getCallStatusImage.css('display', 'none');
      getCallTimer.css('visibility', 'visible');
}
...
$('.video-call-screen').off('click').on('click', function(event) {
  var getCallingUserName = $(this).parents('.chat-system').find('.person.active .user-name').attr('data-name');
  var getCallingUserImg = $(this).parents('.chat-system').find('.person.active .f-head img').attr('src');
      $(this).parents('.chat-box').find('.overlay-video-call .user-name').text(getCallingUserName);
      $(this).parents('.chat-box').find('.overlay-video-call .calling-user-img img').attr('src', getCallingUserImg);
...
$('.mail-write-box').on('keydown', function(event) {
    if(event.key === 'Enter') {
        ...
        $messageHtml = '<div class="bubble me">' + chatMessageValue + '</div>';
         $(this).parents('.chat-system').find('.active-chat').append($messageHtml);
        const getScrollContainer = document.querySelector('.chat-conversation-box');
