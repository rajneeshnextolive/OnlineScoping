$(function() {
   ...
   Container.card;
    ...
    function $_taskSortable() {
        $('[data-sortable="true"]').sortable({
            ...
            stop: function( event, ui ) {
             ui.item.parent().attr('data-section');
    ...
    $('.addTaskAccordion .collapse').on('shown.bs.collapse', function () {
       ...
       var getClassOfAccordion = $(this).parents('.card').attr('class').split(' ')[1];
       var getClassOfAddTaskBtn = $(this).parents('.modal-content').find('[data-btnfn="addTask"]').attr('class').split(' ')[1];
       var removeClassOfAddTaskBtn = $(this).parents('.modal-content').find('[data-btnfn="addTask"]').removeClass(getClassOfAddTaskBtn);
    ...
    function $_taskAdd( getParent ) {
        ...
        $('[data-btnfn="addTask"]').off('click').on('click', function(event) {
           ...
           var getAddBtnClass = $(this).attr('class').split(' ')[1];
            ...
            if (getAddBtnClass === 'task-simple') {
                ...
                var $html =   '<div data-draggable="true" class="card simple-title-task" style="">'+
                                '<div class="card-body">'+
        ...
        $('.list-edit').off('click').on('click', function(event) {
            ...
            $('.edit-list').off('click').on('click', function(event) {
                ...
                var $_getListTitle = document.getElementById('s-list-name').value;
                 ...
                 $_outerThis.parents('[data-connect="sorting"]').find('.s-heading').html($_getListTitle);
               $_outerThis.parents('[data-connect="sorting"]').find('.s-heading').attr('data-listTitle', $_getListTitle);
      ...
      $('.card .s-task-edit').off('click').on('click', function(event) {
        ...
        if ($(this).parents('.card').hasClass('img-task')) {
           ...
           $('.task-text-progress #s-task').val($_taskTitle);
            ...
            var $_taskProgress = $_outerThis.parents('.card').find('div.progress-bar').attr('data-progressState');
             $('#progress-range-counter').val($_taskProgress);
            $('.range-count-number').html($_taskProgress);
            $('.range-count-number').attr('data-rangecountnumber', $_taskProgress);
        ...
        $('[data-btnfn="editTask"]').off('click').on('click', function(event) {
            ...
            if ($_outerThis.parents('.card').hasClass('img-task')) {
                ...
                var $_task = document.getElementById('s-image-task').value;
                 $_outerThis.parents('.card').find('h4').attr('data-taskTitle' , $_task);
                 $_outerThis.parents('.card').find('h4').html($_task);
                ...
                $_outerThis.parents('.card').find('h4').attr('data-taskTitle' , $_taskValue);
                $_outerThis.parents('.card').find('h4').html($_taskValue);
                $_outerThis.parents('.card').find('p:not(".progress-count")').attr('data-tasktext' , $_taskTextValue);
                 $_outerThis.parents('.card').find('p:not(".progress-count")').html($_taskTextValue);

                 $_outerThis.parents('.card').find('div.progress-bar').attr('style', "width: " + $_taskProgressValue +"%");
                $_outerThis.parents('.card').find('div.progress-bar').attr('data-progressState', $_taskProgressValue);
                $_outerThis.parents('.card').find('div.progress-bar').attr('aria-valuenow', $_taskProgressValue);
                 $_outerThis.parents('.card').find('.progress-count').html($_taskProgressValue+"%");
            }
